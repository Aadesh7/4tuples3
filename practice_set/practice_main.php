...
<?php
	...
	if(isset($_POST['mybutton'])){
		$val = $_POST['mybutton'];
		$_SESSION['dbname'] = $val;				// Here, database name is accessed from the button that is clicked by the user in the previous page.
	}
	include 'dbconn.php';
...
<!--/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
	<?php
		...
		if($dbname == 'quest'){														//{ Here, according to database accessed, type of question is presented to user
		if(!isset($_SESSION['pcount'])){
				...
				$(document).ready(function(){
				  ...
				  $("#scroll").scroll(function(){
				    ...
				    $("#cick_me").click(function(){
				      if(num1  >  lastScroll){
				        $(location).attr("href","practice_passage_redirect.php");
				      }
				      else {
				        alert("not good");
				        return false;
			...
			if($count%13 != 0){																			// Here, table needs to be accessed according to count ie id because of different question formats
																										// normal mcqs, question no 12 and question no 13 are three tables which needs to be accessed like this.
				if ($count%12 == 0){
					$table_name = 'table_2';
				...
				<?php
					if(isset($_POST['send'])){
						if($_POST['send']==1){
							echo '<script>$("#first").show();</script>';								// This is used for qno. 12.
						}
						if($_POST['send']==2){
				...
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0){
					while ($row = mysqli_fetch_assoc($result)) {													// This block accesses database to retrieve questions.
						echo "<p>";
						...
						echo '<form action="practice_redirect.php" method="post" id="ques_form">';
  							...
  							echo '<input type="submit" class="button" name="submit" value="next" id="next_button"/>'; 				//submitted to redirect page which will also check if ans is correct and store in DB.
				...
				if (mysqli_num_rows($result) > 0){
					...
					?>
						<form action="practice_redirect.php" method = "post" id="myform" onsubmit="return getData();">
							<div id="div2" name="div2" ondrop="drop(event, this)" ondragover="allowDrop(event)" style= "background-color: red;"></div><br>
					...
					<?php
  						echo  '<div id ="div3" ondrop="drop(event,this)" ondragover  = " allowDrop(event)">';
            				...
            				echo '<p id="drag3" draggable="true" ondragstart="drag(event)">'.$row['ans3'];'</p>';				//This is the drag and drop effect which is implemented using jscript (in header)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
		elseif($dbname == 'quest2'){																						//Incase of different DB accessed, same process is repeated but using different DB.
			if(!isset($_SESSION['pcount'])){
				$_SESSION['pcount'] = 1;
				$_SESSION['rcount'] = 1;
			}
			...
			<?php	
			echo  '<div id ="div3" ondrop="drop(event,this)" ondragover  = " allowDrop(event)">';
				echo '<p id="drag1" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans1'];'</p>';
            	echo '<p id="drag2" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans2'];'</p>';
            	echo '<p id="drag3" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans3'];'</p>';
            	echo '<p id="drag4" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans4'];'</p>';
            	echo '<p id="drag5" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans5'];'</p>';
            	echo '<p id="drag6" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans6'];'</p>';
            	echo '<p id="drag7" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans7'];'</p>';
            	echo '<p id="drag8" draggable="true" class="icon" ondragstart="drag(event)">'.$row['ans8'];'</p>';
            echo '</div>';	
				echo '</div>';
                echo '</div>';
                echo '</div>';
